...
def main(args):
    ...
    # saving test
    torch.save(G, '/work/310613060/two_stage/Cross_Validation_Dice/G_D/checkpoints16-17/generator_0.pth')
    ...
    save_image(g[0, 0, 32, :, :], '/work/310613060/two_stage/Cross_Validation_Dice/G_D/checkpoints16-17/G0.png')
    ...
    for epoch in range(args.epochs):
        ...
        G_Loss.append(g_l)
        if (epoch+1) % 10 == 0:
            torch.save(G, '/work/310613060/two_stage/Cross_Validation_Dice/G_D/checkpoints16-17/generator_'+str(epoch+1)+'.pth')
            # torch.save(D, './checkpoints/discriminator_'+str(epoch+1)+'.pth')
            testing_image = torch.unsqueeze(torch.tensor(training_set[100][0]), 0)
            g = G(testing_image.cuda().float())
            save_image(g[0, 0, 32, :, :], '/work/310613060/two_stage/Cross_Validation_Dice/G_D/checkpoints16-17/G'+str(epoch+1)+'.png')
    ...
    plt.legend()
    plt.savefig('loss_GD_test16-17.png')
