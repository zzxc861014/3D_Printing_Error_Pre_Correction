...
class Generator(nn.Module):
    def __init__(self, in_ch, out_ch, ngf=64):
        ...
        self.de1 = nn.Sequential(
            ...
            nn.ConvTranspose3d(ngf * 8 + 30, ngf * 8, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm3d(ngf * 8),
            nn.Dropout(p=0.5)
        )
        # 2 * 2 * 2
    ...
    def forward(self, X, coord):
        ...
        #
        en6_out = torch.cat([en6_out, coord], dim=1)
...
def D_train(D: Discriminator, G: Generator, X, Y, coord, BCELoss, optimizer_D, device='cuda:0'):
    """
...
# training generator
def G_train(D: Discriminator, G: Generator, X, Y, coord, DiceLoss, L1, optimizer_G, lamb=100, device='cuda:0'):
    """
    :param D: Discriminator
    :param G: Generator
    :param X: input images
